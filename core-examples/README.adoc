...
The `Gateway` deploys two instances of the `HttpServerVerticle` verticle.
...
* Start the server by executing the `main` method of the `Gateway` class
...
In a terminal, find the process related to the `Gateway` class execution and kill it using `kill -9`. The verticle is
